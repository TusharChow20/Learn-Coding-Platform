    ...
    <div className={`min-h-screen flex items-center justify-center p-4 transition-colors duration-500 ${
      ...
      <button
        ...
        className={`absolute top-6 right-6 p-3 rounded-full transition-all duration-300 hover:scale-110 ${
          isDark ? 'bg-slate-800 text-yellow-400' : 'bg-white text-purple-600 shadow-lg'
        }`}
      >
        {isDark ? '☀️' : '🌙'}
      </button>
      ...
      <div className={`relative w-full max-w-md transition-all duration-500 ${
        ...
        <div className="flex justify-center mb-8">
          <div className={`relative p-4 rounded-2xl ${
            isDark ? 'bg-gradient-to-br from-purple-600 to-blue-600' : 'bg-gradient-to-br from-purple-500 to-pink-500'
          } shadow-lg animate-bounce`} style={{ animationDuration: '3s' }}>
            <Sparkles className="w-8 h-8 text-white" />
            <div className="absolute inset-0 rounded-2xl bg-white/20 animate-ping" style={{ animationDuration: '2s' }}></div>
          </div>
        </div>
          ...
          <div className="relative group">
            ...
            <div className="relative">
              <Mail className={`absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 transition-colors ${
                isDark ? 'text-slate-400 group-hover:text-purple-400' : 'text-gray-400 group-hover:text-purple-500'
              }`} />
              <input
                type="email"
                value={email}
            ...
            <div className="relative">
              ...
              <button
                ...
                className={`absolute right-4 top-1/2 -translate-y-1/2 transition-colors ${
                  isDark ? 'text-slate-400 hover:text-purple-400' : 'text-gray-400 hover:text-purple-500'
                }`}
              >
        ...
        <div className="relative my-8">
          ...
          <div className="relative flex justify-center text-sm">
            <span className={`px-4 ${
              isDark ? 'bg-slate-800/50 text-slate-400' : 'bg-white/80 text-gray-500'
            }`}>
              Or continue with
            </span>
          </div>
